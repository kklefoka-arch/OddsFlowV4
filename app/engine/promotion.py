"""OddsFlow V3 — Foundation Matrix and Promotion Framework.

Computes per-cell hit rates and applies promotion logic for goals,
corners, and 3-way markets across tier splits (all / T1 / T2T3).
"""
...
def _threeway_green(row: dict, zone: str) -> bool:
    ...
    if zone in ("strong", "standard"):
        return alpha_wins or draw   # DNB
    else:
        return alpha_wins           # outright win only
...
def _threeway_pick_label(zone: str) -> str:
    """Human-readable pick label for the 3-way market."""
    return "DNB (Alpha Win or Draw)" if zone in ("strong", "standard") else "Alpha Win"
...
def _compute_cells(rows: list[dict]) -> list[dict[str, Any]]:
    """Build per-(zone × df × bts_pocket) cell statistics from a list of fixture rows.

    V3.1 (2026-05-27): partition key extended with DF (Difference Factor).
...
def compute_foundation(rows: list[dict]) -> dict[str, Any]:
    ...
    Produces three tier splits (all / t1 / t2t3) each containing a list of
    ...
    """
    t1_rows = [r for r in rows if r.get("tier") == 1]
    t2t3_rows = [r for r in rows if r.get("tier") in (2, 3)]
